...
void print(int x, int y, char* s, int color) { 
    int pos = y * 80 + x; 
    for(int i = 0; s[i]; i++) vga[pos+i] = (color << 8) | s[i]; 
} 
void draw_box(int x, int y, int w, int h, int c) { 
    for(int i = 0; i < w; i++) { 
        vga[y*80+x+i] = (c<<8)|196; 
        vga[(y+h-1)*80+x+i] = (c<<8)|196; 
    } 
    for(int i = 0; i < h; i++) { 
        vga[(y+i)*80+x] = (c<<8)|179; 
        vga[(y+i)*80+x+w-1] = (c<<8)|179; 
    } 
    vga[y*80+x] = (c<<8)|218; 
    vga[y*80+x+w-1] = (c<<8)|191; 
    vga[(y+h-1)*80+x] = (c<<8)|192; 
    vga[(y+h-1)*80+x+w-1] = (c<<8)|217; 
} 
void kmain(void) { 
    clear_screen(); 
    draw_box(5, 2, 70, 20, 0x09); 
    print(15, 4, "LilithOS - Windows 11 Style OS", 0x0F); 
    draw_box(10, 7, 60, 10, 0x0B); 
